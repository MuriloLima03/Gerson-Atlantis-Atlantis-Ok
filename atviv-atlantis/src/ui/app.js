  ...
  document.getElementById('btn-show-types').addEventListener('click', async ()=>{
    try {
      ...
      if (!types || !types.length) {
        alert('Não há tipos de acomodação cadastrados.');
        return;
      }
      const typeList = types.map(t => `${t.key} — ${t.description}`).join('\n');
      alert('Tipos de Acomodação:\n\n' + typeList);
    } catch(e) {
      console.error('Failed to load accommodation types:', e);
      alert(e.message || 'Erro ao carregar tipos de acomodação');
    }
  });
...
async function showAccForm(id) {
  ...
  area.innerHTML = '<div class="loading">Carregando formulário...</div>';
  
  try {
    const [items, types] = await Promise.all([
      id ? load('atlantis_accommodations_v1') : Promise.resolve([]),
      api.getAccommodationTypes()
    ]);

    if (!types || !types.length) {
      throw new Error('Não há tipos de acomodação disponíveis. Por favor, contate o administrador.');
    }

    let a;
    if (id) {
      a = items.find(x => (x.id === id || x._id === id));
      if (!a) throw new Error('Acomodação não encontrada');
    } else {
      a = {
        name: '',
        type: '',
    ...
    document.getElementById('acc-form').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (!name) throw new Error('O nome da acomodação é obrigatório');
        if (!type) throw new Error('O tipo da acomodação é obrigatório');
        if (isNaN(rate) || rate < 0) throw new Error('A tarifa deve ser um valor válido maior ou igual a zero');
        
        const accData = { name, type, rate };
